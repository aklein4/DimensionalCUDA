 ...
 * NOTE: assumes the values of C have been set to zero
 ...
 */
template <class T>
__global__ void gpu_math_first_summation(T* B, T* C, int width, int height, int width_C, int sum_divider) {
    // get location
    int i = (blockIdx.x * blockDim.x) + threadIdx.x;
    int j = (blockIdx.y * blockDim.y) + threadIdx.y;
    if (i >= width || j >= height) return;
 ...
 * NOTE: assumes the values of y have been set to zero
 ...
 */
template <class T>
__global__ void gpu_math_final_summation(T* C, T* y, int width, int height) {
    // get location
    int i = (blockIdx.x * blockDim.x) + threadIdx.x;
    int j = (blockIdx.y * blockDim.y) + threadIdx.y;
...
void matMulti(Matrix2D<T> &A, Vector1D<U> &x, Vector1D<V> &y) {
    ...
    DIM_CUDA_HIDDEN::gpu_math_first_summation<V><<<numBlocks, blockSize>>>(
        B, C, static_cast<int>(A.width()), static_cast<int>(A.height()), width_C, SUM_DIVIDER
    );
    ...
    DIM_CUDA_HIDDEN::gpu_math_final_summation<V><<<numBlocks, blockSize>>>(
        C, y.get_data(), width_C, static_cast<int>(y.size())
    );
